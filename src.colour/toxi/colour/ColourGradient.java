...
/**
 * This class can be used to calculate multi-colour gradients with colours
...
public class ColourGradient {
	...
	/**
	 * Adds a new colour at specified position.
	 * 
	 * @param p
	 * @param c
	 */
	public void addColorAt(float p, Colour c) {
	 ...
	 * @param pos
	 * @param width
	 * @return list of interpolated gradient colours
	 */
	public ColourList calcGradient(float pos, int width) {
		ColourList result = new ColourList();
		...
		while (pos < endPos) {
			...
			if (currPoint != activeGradient.last()) {
				...
				result.add(currPoint.colour.getBlended(nextPoint.colour,
						ditheredFrac));
			} else {
				result.add(currPoint.colour.copy());
			}
			pos++;
		}
		return result;
	 ...
	 * interpolated colours in the calculated gradient. The value range for this
